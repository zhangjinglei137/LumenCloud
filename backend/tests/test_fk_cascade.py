...
async def _seed_cascade_media_with_tq():
    """媒体 + task_queue + 引用 task_queue 的 download_queue（FK 链 CASCADE 双跳）。"""
    from app.database import async_session
    from app.models import DownloadQueue, DownloadTask, EpisodeState, Media, TaskQueue

    async with async_session() as session:
        media = Media(title="FK链级联测试", status="tracking", in_emby=False)
        session.add(media)
        await session.flush()
        tq = TaskQueue(media_id=media.id, episode="S01E01", status="done")
        session.add(tq)
        await session.flush()
        session.add_all(
            [
        ...
        await session.commit()
        return {"media_id": media.id, "task_queue_id": tq.id}


async def _delete_media_only_and_count(media_id: int, task_queue_id=None) -> dict:
    ...
    async with async_session() as session:
        ...
        if task_queue_id is not None:
            counts["download_queue_task_ref"] = await session.scalar(
                select(func.count())
...
def test_delete_media_cascades_download_queue_task_ref():
    """FK 链第二跳：删除 Media → 引用 task_queue 的 download_queue.task_queue_id 级联清空。"""
    with TestClient(app) as client:
        seed = client.portal.call(_seed_cascade_media_with_tq)
        counts = client.portal.call(
            _delete_media_only_and_count, seed["media_id"], seed["task_queue_id"]
        )
        assert counts == {
            "episode_state": 0,
            "task_queue": 0,
