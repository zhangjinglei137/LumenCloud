...
from app.models import DownloadQueue, DownloadTask, EpisodeState, Media, Notification, TaskRun, TransferQueue, WatchRequest
...
async def seed_done_state(db, *, episode="S01E01", file_name="ep.mkv", dl_status="complete",
    ...
    仅保留签名兼容；resolve 按 status='done' 判定）。media_id 传入时复用既有 media。
    """
    async with db() as s:
        if media_id is None:
        ...
        dq = DownloadQueue(
            ...
            retry_count=retry_count, node_attempt=0, updated_at=_now(),
        )
        s.add(dq)
        await s.flush()
        await s.commit()
...
def test_done_resolution_deletes_when_emby_confirmed(db, monkeypatch):
    """tv：Emby missing 不含该集 → 已确认入库 → 删除 download_queue（防重解除）。"""
    from app.tasks import scan as scan_mod

    monkeypatch.setattr(scan_mod, "async_session", db)
    mid, dq_id = run(seed_done_state(db, episode="S01E01"))
...
def test_done_resolution_marks_failed_when_still_missing(db, monkeypatch):
    """tv：Emby missing 仍含该集 → 转 failed 人工确认（download_queue 单表）。"""
    from app.tasks import scan as scan_mod
    ...
    assert dq.error == "下载完成但 Emby 未入库，请人工确认"
...
def test_done_resolution_movie_missing_marks_failed(db, monkeypatch):
    ...
    monkeypatch.setattr(scan_mod, "async_session", db)
    mid, dq_id = run(seed_done_state(db, episode="电影.mkv"))
...
def test_done_resolution_keeps_other_episodes(db, monkeypatch):
    ...
    monkeypatch.setattr(scan_mod, "async_session", db)
    mid, dq_id = run(seed_done_state(db, episode="S01E01"))
    _, dq2_id = run(seed_done_state(db, episode="S01E02", media_id=mid))
    run(scan_mod._resolve_done_states(types.SimpleNamespace(id=mid), {"S01E02"}, False))
...
def test_cleanup_removes_unreferenced_complete(db, monkeypatch):
    ...
    mid, dq_id = run(seed_done_state(db, dl_status="complete"))
...
def test_cleanup_keeps_downloading_referenced(db, monkeypatch):
    """downloading（download_queue 非终态）的文件仍受保护（不误删进行中任务）。

    影视下载两队列重设计后 cleanup 引用集合 = download_queue 非终态行的
    quark_path/file_name（§P1-3 语义迁移：终态不再保护，非终态一律保护）。
    """
    from app.tasks import cleanup as cleanup_mod
    ...
    async def _seed():
        async with db() as s:
            ...
            s.add(media)
            await s.flush()
            dq = DownloadQueue(media_id=media.id, episode="S01E01", file_name="ep.mkv",
                               file_size=1024, share_code="sc", stoken="st",
                               receive_code="rc", fids="[]", fid_tokens="[]",
                               folder_id="fd", status="downloading", quark_path="/quark/ep.mkv",
                               updated_at=_now())
            s.add(dq)
            await s.commit()
    run(_seed())
...
async def seed_pending(db, *, episode="S01E01", file_name="ep.mkv", stoken="stoken-x"):
    ...
    async with db() as s:
        ...
        dq = DownloadQueue(
            media_id=mid, episode=episode, file_name=file_name, file_size=1024,
            share_code="sc", stoken=stoken, receive_code="提取码占位",
...
def test_transfer_failure_backoff_triggers_resume(db, monkeypatch):
    ...
    # 新语义（§4.2）：失败回退不再 _spawn 续跑——_admit_batch 对失败回退 break 本批，
    # 由下一轮 job/事件续跑（等价原版「一次处理一个 + 续跑」）。断言转为：
    # 首次调用回退 pending（retry_count+1）→ 再次调用成功推进 downloading。
    async def scenario():
        # 第 1 轮：save 失败（RuntimeError）→ 非终态回退 pending，本批停止
        await transfer_mod.process_transfer_queue()
        # 第 2 轮：续跑成功 → 推进 downloading
        await transfer_mod.process_transfer_queue()

    run(scenario())

    dq = run(read_row(db, DownloadQueue, dq_id))
    assert dq.retry_count == 1  # 仅首次失败消耗 retry
    assert dq.status == "downloading"  # 续跑成功
    assert len(cloud.save_calls) == 2  # 首败 + 续跑成功各一次
...
def test_done_resolution_failed_respects_retry_count_below_limit(db, monkeypatch):
    """P3-1：未达上限（retry_count=2 < 3）→ 转 failed 并发推进位（retry_count +1）。"""
    from app.tasks import scan as scan_mod

    monkeypatch.setattr(scan_mod, "async_session", db)
    mid, dq_id = run(seed_done_state(db, episode="S01E01", retry_count=2))
    run(scan_mod._resolve_done_states(types.SimpleNamespace(id=mid), {"S01E01"}, False))

    dq = run(read_row(db, DownloadQueue, dq_id))
    assert dq.status == "failed"
    assert dq.retry_count == 3  # done→failed 算一次循环，retry_count SQL 自增
    assert dq.error == scan_mod._DONE_FAIL_ERROR


def test_done_resolution_hit_retry_limit_marks_failed(db, monkeypatch):
    """P3-1/P1-5：达循环上限（retry_count=3）→ 写 status='failed' 终态
    （与其它失败终态对齐，queue.py retry_task 可人工解锁；此前保持 done 仅写 error
    导致 retry 只认 failed 无法干预而卡死），上限分支不联动其他行（retry 时再由
    queue.py 单表重置）。"""
    from app.tasks import scan as scan_mod

    monkeypatch.setattr(scan_mod, "async_session", db)
    mid, dq_id = run(seed_done_state(db, episode="S01E01", retry_count=3))
    run(scan_mod._resolve_done_states(types.SimpleNamespace(id=mid), {"S01E01"}, False))

    dq = run(read_row(db, DownloadQueue, dq_id))
    assert dq is not None
    assert dq.status == "failed"
    assert dq.retry_count == 3  # 不消耗 / 不再递增
    assert dq.node_attempt == scan_mod._DONE_FAIL_RETRY_LIMIT  # 达上限值
    assert dq.node_error == scan_mod._DONE_LIMIT_ERROR  # 上限 error 文案，人工核实 Emby 端
...
def test_done_resolution_limit_error_not_rewritten(db, monkeypatch):
    ...
    assert dq1.error == scan_mod._DONE_LIMIT_ERROR
    first_ts = dq1.updated_at  # 首次写入 error 时的时间戳

    run(scan_mod._resolve_done_states(types.SimpleNamespace(id=mid), {"S01E01"}, False))
    dq2 = run(read_row(db, DownloadQueue, dq_id))
    # 第二轮不再改写：error 保持上限文案、updated_at 未被污染
    assert dq2.error == scan_mod._DONE_LIMIT_ERROR
    assert dq2.updated_at == first_ts