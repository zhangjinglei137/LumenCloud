...
- [ ] 2.1 放宽 GID 白名单口径为「DB 中 aria2_gid 非空全部行」（不再限定 downloading），并验证现有 GID 校验测试更新后通过
...
- [ ] 8.1 刮削连坐风暴：`scrape_runner` 失败后对 media 设置进程内退避（10min），同步失败与节点重试计数解耦，并验证新增测试覆盖「NasTools 故障不批量累加全部 scrape 行 node_attempt」
- [ ] 8.2 `aria2.add_uri` 追加 `allow-overwrite=true` + `auto-file-renaming=false` options（先确认生产 aria2 启动参数，若已配置则跳过并记录），并验证 aria2 RPC shape 测试更新通过
