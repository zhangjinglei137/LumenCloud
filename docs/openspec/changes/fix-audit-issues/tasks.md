...
- [ ] 3.1 抽象登录限流为通用限流器，注册接口对邀请码错误计数限流（窗口超限 429），验证新增注册限流测试通过且正常注册不受影响
- [ ] 3.2 登录用户不存在时对 dummy hash 执行等价校验抹平时序侧信道，验证新增时序等价测试通过
- [ ] 3.3 users 表新增 `token_version`，JWT payload 带 `ver`、`get_current_user` 校验，改密时递增，验证改密后旧 token 返回 401
- [ ] 3.4 新增/改造登出接口清除 httpOnly cookie，前端 logout 先调登出再清 localStorage，验证登出后残留 cookie 无法通过鉴权
- [ ] 3.5 登录 redirect 参数白名单校验（`/` 开头且非 `//`），非法回退首页，验证新增跳转安全测试通过
