...
- [x] 3.1 调查「status=library 迟迟不入库」根因（find_emby_id 未命中/遗漏集盲匹配/超时配置/scheduler 注册缺失，逐项核查日志与代码），记录根因结论
- [x] 3.2 修复：library_check 的 continue 分支补充明确日志（记录不 finalize 的具体原因），并让超时判定覆盖「Emby 命中但遗漏集判定」路径（避免无限等待），验证卡死可诊断/可超时
- [x] 3.3 补充测试：遗漏集判定终态、超时路径用例，验证 `pytest` 通过
...
- [x] 5.1 手动验证：巡检队列命名/字段/正序分页、下载队列分享码跳转、无展开更多、入库确认不再长期卡死，验证 `npm run build` 与后端启动无报错
