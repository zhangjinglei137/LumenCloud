...
- 同步修正过时文档：模块 docstring（已移 query 通道的事实沿革）、`config.py` 注释、设计文档 §12.2/§12.4 的配置指导（删除 `?token=` 引导，改为新版渠道 Authorization 裸 token 说明）
...
- **文档**：`docs/影视下载两队列重设计.md` §12.2/§12.4、`backend/app/config.py` 注释、模块 docstring
